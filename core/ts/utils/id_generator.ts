...
export class IDGenerator {
    ...
    public static reset() { IDGenerator.idCount = 0; }
    ...
    public static applyID(elem: Element) { elem.id = elem.id ? elem.id : IDGenerator.next; }
     ...
     */
    public static applyRecur(elem: Element | string) { D.eachRecur(elem, this.applyID); }

    /**
     * Attaches ids to all html elements in the target location in the DOM that do not have ids.
     */
    public static attachIdsToAllElements() { IDGenerator.applyRecur(Elements.htmlLoc); }
