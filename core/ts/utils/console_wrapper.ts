...
export async function error <T>(func: (...args: any) => T): Promise<T | undefined> {
    return errorHO(func)();
}

export function errorHO <T>(func: (...args: any) => T): () => Promise<T | undefined> {
    return async () => {
        try {
            return func();
        } catch (error) {
...
export function setDebugLevel(level: DebugLevelEnum) { debugLevel = level; }
